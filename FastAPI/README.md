...
```bash
# Перейти по короткой ссылке (curl следует редиректу и возвращает тело конечной страницы)
curl -L "http://localhost:8000/api/v1/links/abc123"

# Только посмотреть, куда ведёт ссылка (заголовок Location), без перехода
...
Переменные окружения задаются в `docker-compose.yml`; для локального запуска без Docker скопируйте `.env.example` в `.env` и укажите `DATABASE_URL` и `REDIS_URL`.
