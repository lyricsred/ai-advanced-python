...
- **Пересчёт:** из каталога `FastAPI` выполните:
  ```bash
  pytest tests --cov=app --cov-report=html --cov-report=term
  ```
  Новый отчёт будет в каталоге `htmlcov/`. Чтобы снова закоммитить отчёт, при необходимости удалите `htmlcov/.gitignore` (его создаёт coverage) и выполните `git add htmlcov`.
...
Установка: `pip install locust` (или из `requirements.txt`).

**Перед запуском** поднимите API (например `docker compose up` или `uvicorn app.main:app --reload`).

### Запуск Locust

- **Веб-интерфейс** (по умолчанию http://localhost:8089):
  ```bash
  locust -f locustfile.py --host=http://localhost:8000
  ```
  Укажите число пользователей и RPS, запустите тест и смотрите графики/таблицы.

- **Без веб-интерфейса** (результаты в консоль):
  ```bash
  locust -f locustfile.py --host=http://localhost:8000 --headless -u 20 -r 5 -t 1m
  ```
  `-u` — число пользователей, `-r` — скорость появления пользователей в сек, `-t` — длительность.

- **Отчёт в HTML:**
  ```bash
  locust -f locustfile.py --host=http://localhost:8000 --headless -u 30 -r 10 -t 2m --html=load_report.html
  ```

### Сценарии в locustfile.py

- **ShortenerUser** — создание ссылок, редирект и статистика. Каждый пользователь создаёт одну ссылку при старте и многократно обращается к ней (редирект и stats), плюс создаёт новые ссылки.
- **CreateOnlyUser** — только массовое создание ссылок (максимальная нагрузка на POST `/shorten`).

При выборе класса в веб-интерфейсе Locust укажите нужный User.

### Влияние кэширования

Редирект (`GET /api/v1/links/{short_code}`) и статистика (`GET /api/v1/links/{short_code}/stats`) кэшируются в Redis. Чтобы оценить эффект:

1. Запустите тест **с Redis** (обычный запуск через Docker / с `REDIS_URL`). В отчёте Locust посмотрите среднее время ответа и RPS для запросов `/redirect` и `/stats`.
2. Запустите тест **без Redis** (например, отключите Redis в `docker-compose` или подставьте нерабочий `REDIS_URL` и перезапустите API). Сравните задержки и RPS.

При включённом кэше повторные запросы к одной и той же ссылке должны обрабатываться быстрее (меньше обращений к БД), что видно по снижению среднего времени ответа и росту RPS для сценария с частыми редиректами/статистикой.
