...
if uploaded_file is not None:
    ...
    with col3:
        st.metric(
            "Период данных", f"{data['timestamp'].min().date()} - {data['timestamp'].max().date()}"
        )
            ...
            with st.spinner("Выполняется анализ данных..."):
                ...
                for city in results:
                    if "data" in results[city] and "timestamp" in results[city]["data"].columns:
                        ...
                        results[city]["data"]["timestamp"] = results[city]["data"][
                            "timestamp"
                        ].astype(str)
            ...
            if "parallel_performance" in st.session_state:
                ...
                if perf["sequential"] > 0:
                    ...
                    fastest_name = {
                        ...
                        "parallel_thread": "Параллельный (потоки)",
                    }[fastest_method[0]]
            ...
            if api_key:
                    ...
                    if st.button("Получить текущую температуру"):
                        with st.spinner("Запрос к API..."):
                            if method == "Синхронный":
                                current_temp, error = get_current_temperature_sync(
                                    selected_city, api_key
                                )
                            else:
                                current_temp, error = get_current_temperature_async_wrapper(
                                    selected_city, api_key
                                )
                ...
                with col2:
                    if st.button("⚡ Сравнить производительность методов API"):
                        ...
                        with st.spinner(
                            f"Сравнение производительности для {len(test_cities)} городов..."
                        ):
                ...
                if "current_temp" in st.session_state:
                    ...
                    if error:
                        ...
                        st.success(
                            f"✅ Текущая температура в {selected_city}: {current_temp:.2f}°C"
                        )
                        ...
                        if min_temp is not None and max_temp is not None:
                            ...
                            if is_normal:
                                st.success(
                                    f"✅ Температура в пределах нормы для сезона ({min_temp:.2f}°C - {max_temp:.2f}°C)"
                                )
                            else:
                            ...
                            st.warning(
                                "⚠️ Не удалось определить нормальный диапазон для данного сезона"
                            )
            else:
