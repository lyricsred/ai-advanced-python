...
def plot_temperature_timeseries(data: pd.DataFrame, city: str, 
                               show_anomalies: bool = True) -> plt.Figure:
    fig, ax = plt.subplots(figsize=(12, 6))
    ax.plot(data['timestamp'], data['temperature'], 
            color='lightblue', linewidth=1, label='Температура', alpha=0.7)
    
    if 'moving_avg' in data.columns:
        ax.plot(data['timestamp'], data['moving_avg'], 
                color='blue', linewidth=2, label='Скользящее среднее (30 дней)')
    
    if show_anomalies and 'anomaly' in data.columns:
        anomalies_data = data[data['anomaly']]
        if len(anomalies_data) > 0:
            ax.scatter(anomalies_data['timestamp'], anomalies_data['temperature'],
...
def plot_seasonal_profiles(seasonal_stats: pd.DataFrame, city: str) -> plt.Figure:
    ...
    ax.plot(seasons, lower_bound, '--', color='lightblue', 
           linewidth=1.5, label='Нижняя граница (±2σ)')
...
def plot_anomaly_timeline(data: pd.DataFrame, city: str) -> plt.Figure:
    ...
    ax.bar(monthly_anomalies['year_month'], monthly_anomalies['anomaly'],
