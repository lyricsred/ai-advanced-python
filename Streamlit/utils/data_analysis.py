...
def detect_anomalies(data: pd.DataFrame, moving_avg: pd.Series, 
...
def analyze_city_data(city_data: pd.DataFrame, window: int = 30, 
...
def analyze_data_sequential(data: pd.DataFrame, window: int = 30, 
...
def analyze_data_parallel(data: pd.DataFrame, window: int = 30, 
                         threshold_std: float = 2.0, 
                         method: str = 'process') -> Dict[str, Dict]:
    cities = data['city'].unique()
    args_list = [
        (city, data[data['city'] == city].copy(), window, threshold_std)
        for city in cities
    ]
...
def compare_parallelization_performance(data: pd.DataFrame, window: int = 30, 
                                      threshold_std: float = 2.0) -> Dict[str, float]:
    performance = {}
...
def get_seasonal_normal_range(seasonal_stats: pd.DataFrame, city: str, 
